...
Ruby wallet provides full access to the bitcoind JSON-RPC API, along with abstractions to better organize the features. For a full list of all implemented methods see [API Method List](https://en.bitcoin.it/wiki/Original_Bitcoin_client/API_Calls_list).
...
After doing this, the simplest possible usage looks like this:
  ...
  RubyWallet.connect('username', 'password').balance
  # => 0.001
...
You can specify connection information for remote clients.
  ...
  wallet = RubyWallet.connect('username', 'password', :host => 'example.com', :port => 38332, :ssl => true)
